...
from collectors.activity_logs import collect_activity_logs
...
def _split_personal_focus(personal_sessions: list[dict]) -> tuple[int, int]:
    home = group = 0
...
def collect_for_date(target_date: date) -> dict:
    engine = get_readonly_engine()

    group_data = collect_group_participation(engine, target_date)
    timer_data = collect_timer_usage(engine, target_date)
    activity_data = collect_activity_logs(engine, target_date)

    personal_sessions = timer_data["personal_sessions"]
    group_sessions = timer_data["group_sessions"]
    personal_home, personal_group = _split_personal_focus(personal_sessions)
    personal_focus = personal_home + personal_group
    ...
    return {
        ...
        "summary": {
            ...
            "invitations_responded_count": len(group_data["invitations_responded"]),
            "personal_session_count": len(personal_sessions),
            "group_session_count": len(group_sessions),
            "personal_home_focus_seconds": personal_home,
            "personal_group_focus_seconds": personal_group,
            "personal_focus_seconds": personal_focus,
            "group_focus_seconds": _sum_focus_seconds(group_sessions),
            "group_stay_seconds": None,
            "activity_log_count": int(activity_data.get("total_events") or 0),
            "activity_unique_users": int(activity_data.get("unique_users") or 0),
        },
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--date",
        dest="target_date",
        help="수집 대상 날짜 YYYY-MM-DD (기본: 어제 KST)",
    )
    args = parser.parse_args()
    target_date = (
        date.fromisoformat(args.target_date) if args.target_date else get_target_date()
    )
    print(f"수집 대상 날짜 (KST): {target_date}")

    payload = collect_for_date(target_date)
